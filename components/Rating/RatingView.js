...
export default class RatingView extends React.Component {
  ...
  handleSubmit = () => {
    if (
      this.state.movieName.length >= 3 &&
      this.state.movieComment.length >= 3
    ) {
      let movie = {
  ...
  render() {
    const widget = this.state.isCreating ? (
      <View>
        <Text style={styles.label}>Nom du film:</Text>
        <TextInput
          style={styles.input}
        ...
        />
        <Text style={styles.label}>Note(0-9):</Text>
        <TextInput
          style={styles.input}
      ...
      <View>
        ...
        <View style={styles.row}>
          <View>
            <Text style={styles.item}>Nom du film | Commentaire | Note</Text>
          </View>
            ...
            renderItem={({ item }) => (
              <View style={styles.row}>
                <Text style={styles.item}>
                  {item.name} | {item.comment} | {item.rating}
                </Text>
              </View>
            )}
          />
